## Makefile
## Summary: Cross-compilation builder for ngram.
##
## Author:  KaisarCode
...
NATIVE_EXE_EXT :=
NATIVE_SHARED_NAME := libngram.so
NATIVE_IMPORT_LIBRARY :=

ifeq ($(NATIVE_PLATFORM),windows)
NATIVE_EXE_EXT := .exe
NATIVE_SHARED_NAME := libngram.dll
NATIVE_IMPORT_LIBRARY := -DNGRAM_TEST_IMPORT_LIBRARY=$(CURDIR)/$(BIN_DIR)/$(NATIVE_TARGET)/libngram.dll.a
endif
...
define macos_target
	...
	@cp $(BUILD_DIR)/$(1)-macos/out/ngram $(BIN_DIR)/$(1)/macos/ngram
	@echo "OK $(1)/macos"
endef
...
define ios_target
	...
	@if [ -f $(BUILD_DIR)/$(1)-$(2)/out/ngram ]; then \
		cp $(BUILD_DIR)/$(1)-$(2)/out/ngram $(BIN_DIR)/$(1)/$(2)/ngram; \
	elif [ -f $(BUILD_DIR)/$(1)-$(2)/out/ngram.app/ngram ]; then \
		cp $(BUILD_DIR)/$(1)-$(2)/out/ngram.app/ngram $(BIN_DIR)/$(1)/$(2)/ngram; \
	else \
...
test:
	@if [ -n "$(filter wine,$(MAKECMDGOALS))" ]; then \
		...
		if [ ! -f $(BIN_DIR)/x86_64/windows/libngram.dll ] || [ ! -f $(BIN_DIR)/x86_64/windows/libngram.dll.a ] || [ ! -f $(BIN_DIR)/x86_64/windows/ngram.exe ]; then \
		...
		if [ ! -f $(BUILD_DIR)/test-wine/CMakeCache.txt ]; then \
			cmake -S . -B $(BUILD_DIR)/test-wine \
				...
				-DNGRAM_BUILD_TESTS=ON \
				-DNGRAM_TEST_SHARED_LIBRARY=$(CURDIR)/$(BIN_DIR)/x86_64/windows/libngram.dll \
				-DNGRAM_TEST_IMPORT_LIBRARY=$(CURDIR)/$(BIN_DIR)/x86_64/windows/libngram.dll.a \
				-DNGRAM_TEST_CLI=$(CURDIR)/$(BIN_DIR)/x86_64/windows/ngram.exe \
				-DCMAKE_CROSSCOMPILING_EMULATOR=$(WINE) \
				-G Ninja -Wno-dev > /dev/null; \
		fi; \
		cmake --build $(BUILD_DIR)/test-wine --target ngram_contract_test || exit 1; \
		...
		if [ ! -f $(BIN_DIR)/$(NATIVE_TARGET)/$(NATIVE_SHARED_NAME) ] || [ ! -f $(BIN_DIR)/$(NATIVE_TARGET)/ngram$(NATIVE_EXE_EXT) ]; then \
		...
		if [ ! -f $(BUILD_DIR)/test/CMakeCache.txt ]; then \
			cmake -S . -B $(BUILD_DIR)/test \
				-DCMAKE_BUILD_TYPE=Release \
				-DNGRAM_BUILD_TESTS=ON \
				-DNGRAM_TEST_SHARED_LIBRARY=$(CURDIR)/$(BIN_DIR)/$(NATIVE_TARGET)/$(NATIVE_SHARED_NAME) \
				-DNGRAM_TEST_CLI=$(CURDIR)/$(BIN_DIR)/$(NATIVE_TARGET)/ngram$(NATIVE_EXE_EXT) \
				$(NATIVE_IMPORT_LIBRARY) \
				-G Ninja -Wno-dev > /dev/null; \
		fi; \
		cmake --build $(BUILD_DIR)/test --target ngram_contract_test || exit 1; \
